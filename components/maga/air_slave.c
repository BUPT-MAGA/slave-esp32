...
{
    return slave->tar_temp;
}

int air_get_curtemp(air_slave_t *slave)
{
    return (slave->air_mode == AIR_MODE_HEAT) ? ceil(slave->cur_temp) : floor(slave->cur_temp);
}
...
{
    slave->cur_temp += (slave->air_mode == AIR_MODE_HEAT) ? speed_effect[air_speed + 1] : -speed_effect[air_speed + 1];
    printf("cur_temp:%.2lf\n", slave->cur_temp);
}
...
void air_update_speed(air_slave_t *slave)
{
    ...
    if (slave->air_speed == AIR_SPEED_NONE && diff < 1.0)
        return;
...
static void report_timer_cb(void *arg)
{
    if (air_slave->air_speed == AIR_SPEED_NONE) {
        ws_client_send_report(air_get_curtemp(air_slave), air_get_tartemp(air_slave), AIR_MODE_NONE, AIR_SPEED_LOW);
    }
    else {
        ws_client_send_report(air_get_curtemp(air_slave), air_get_tartemp(air_slave), air_slave->air_mode, air_slave->air_speed);
    }
}
#endif
